        ...
        <div class="px-4 sm:px-6 lg:px-8 max-w-[100%]">
            <div class="bg-white overflow-hidden shadow-sm rounded-sm">
                <div class="p-6 text-gray-900">
                    <h2 class="text-3xl font-bold text-gray-800 mb-6">{{ __('Edit Important Site') }}</h2>

                    <form action="{{ route('important-site.update', $importantSite) }}" method="POST"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="place_name" id="place_name"
                                ...
                                value="{{ old('place_name', $importantSite->place_name) }}" required>
                            @error('place_name')
                            ...
                            <select name="ward_id" id="ward_id"
                                ...
                                @foreach ($wards as $ward)
                                    <option value="{{ $ward->id }}"
                                        {{ old('ward_id', $importantSite->ward_id) == $ward->id ? 'selected' : '' }}>
                            ...
                            <select name="place_type_id" id="place_type_id"
                                ...
                                @foreach ($placeTypes as $type)
                                    <option value="{{ $type->id }}"
                                        {{ old('place_type_id', $importantSite->place_type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="place_description" id="place_description" rows="4"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('place_description', $importantSite->place_description) }}</textarea>
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 font-semibold mb-2">{{ __('Current Photo') }}</label>
                            @if ($importantSite->photo)
                                <div class="flex items-center gap-4 mb-4">
                                    <img src="{{ Storage::url($importantSite->photo) }}"
                                        alt="{{ $importantSite->place_name }}" class="w-32 h-32 object-cover rounded">
                                    <form action="{{ route('important-site.deletePhoto', $importantSite) }}"
                                        method="POST" onclick="return confirm('{{ __('Delete photo?') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            @else
                                <p class="text-gray-500">{{ __('No photo uploaded') }}</p>
                            @endif
                        </div>

                        <div class="mb-4">
                            <label for="photo"
                                class="block text-gray-700 font-semibold mb-2">{{ __('Upload New Photo') }}</label>
                            ...
                            <div class="mt-3 grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="hidden" name="latitude" id="latitude"
                                        value="{{ old('latitude', $importantSite->latitude) }}">
                                </div>
                                <div>
                                    <label
                                    ...
                                    <input type="hidden" name="longitude" id="longitude"
                                        value="{{ old('longitude', $importantSite->longitude) }}">
                                </div>
                            </div>
                        </div>
