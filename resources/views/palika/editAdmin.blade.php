    ...
    <div class="py-12 sm:py-24 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                    {{ __('Edit Palika Admin') }}
                </h2>
                <p class="mt-2 text-sm text-gray-500">
                    {{ __('Update the administrative official details in the municipality system.') }}
                </p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-8 sm:p-10">
                <form action="{{ route('palika.admin.update', $admin->id) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="mt-2">
                                <select id="designation_id" name="designation_id" required
                                    class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-lg bg-gray-50 py-3 px-4 transition-colors">
                                    <option value="" disabled {{ old('designation_id', $admin->designation_id) ? '' : 'selected' }}>
                                        {{ __('Select a Designation') }}</option>
                                    @foreach ($designations as $designation)
                                        @php
                                            $isUsed = in_array($designation->id, $usedDesignations) && $designation->id != $admin->designation_id;
                                        @endphp
                                        <option value="{{ $designation->id }}" {{ $isUsed ? 'disabled' : '' }}
                                            {{ old('designation_id', $admin->designation_id) == $designation->id ? 'selected' : '' }}
                        ...
                        <div class="col-span-1 md:col-span-2 mt-2">
                            ...
                            <div class="mt-4 flex items-center gap-x-8">
                                ...
                                <label for="photo"
                                    ...
                                    @if ($admin->photo)
                                        <img id="image-preview" src="{{ Storage::disk('public')->url($admin->photo) }}" alt="Preview"
                                            class="absolute inset-0 h-full w-full object-cover">
                                        <svg id="upload-icon" class="hidden h-10 w-10 text-gray-400" fill="none"
                            ...
                            <p class="text-xs text-gray-500 mt-4">
                                PNG, JPG, GIF up to 2MB. Leave unchanged to keep the current photo.
                            </p>
                        </div>
