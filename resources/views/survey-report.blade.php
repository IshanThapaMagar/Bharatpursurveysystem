            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="GET" class="mb-8 flex flex-col md:flex-row md:items-end gap-4">
                    <div>
                        ...
                        <select name="ward"
                            class="border rounded-lg p-3 w-full md:w-48 shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        @foreach ($sectionCharts as $chart)
                            ...
                            <div
                                ...
                                @if (!Auth::user()->isDataCollector())
                                    <div
                                        class="mb-6 bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-3 z-10 relative">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox"
                                                class="pin-chart-checkbox rounded text-indigo-600 focus:ring-indigo-500 w-5 h-5"
                                                data-question-id="{{ $questionId }}" {{ $isPinned ? 'checked' : '' }}>
                                            <span
                                                class="text-sm font-semibold text-gray-700">{{ __('Show on Dashboard') }}</span>
                                        </label>
                                        <div class="custom-title-container {{ $isPinned ? '' : 'hidden' }}">
                                            <input type="text"
                                                class="custom-title-input w-full border-gray-200 rounded-lg text-sm p-2 focus:ring-indigo-500 shadow-inner"
                                                placeholder="{{ __('Custom Dashboard Title') }}"
                                                value="{{ $customTitle }}" data-question-id="{{ $questionId }}">
                                            <button
                                                class="save-pin-title text-xs mt-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-medium px-3 py-1.5 rounded-md transition-colors w-full"
                                ...
                                @endif
                                <h4
                                    class="font-bold text-lg mb-6 text-center text-slate-800 leading-tight w-full flex-grow">
                                    {{ $chart['question_text'] }}</h4>
                    ...
                    @empty
                        <div
                            ...
                            <span
                                class="text-lg font-medium">{{ __('No data available for the selected ward.') }}</span>
                        </div>
                    @endforelse
                </div>
            </div>
    ...
    <script>
        (function () {
            ...
            function initializeSurveyReportCharts() {
                ...
                window.surveyReportCharts = {};

                const chartsData = window.surveyReportChartData;
                const colors = [
                ...
                Object.entries(chartsData).forEach(([questionId, data]) => {
                    ...
                    window.surveyReportCharts[questionId] = new Chart(ctx, {
            ...
            // Expose globally so smooth-navigation.js can call it after AJAX
